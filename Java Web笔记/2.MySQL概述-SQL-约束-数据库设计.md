...
### （3）DDL-表操作-创建
![DDL-表操作-创建表.png](DDL-表操作-创建表.png)
### （4）DDL-表操作-数据类型
...
2. 条件
![逻辑运算符1.png](逻辑运算符1.png)
  ![逻辑运算符2.png](逻辑运算符2.png)
代码：
```sql
--条件查询  
-- 1. 查询年龄等于20的员工  
...
### （3） 例子
emp员工表如下：
![外键约束-emp员工表.png](外键约束-emp员工表.png)
dept部门表如下：
![外键约束-dept部门表](外键约束-dept部门表.png)
...
## 2）数据库设计概念
- 数据库设计就是跟据业务系统的具体要求，结合我们所选用的DBMS，为这个业务系统构造出最优的数据存储模型。
...
如图所示：
![数据库设计-多对多.png](数据库设计-多对多.png)
3. 一对一
实现方式：在任意一方加入外键，关联另一方主键，并且设置外键为唯一（UNIQUE）
示例：
![数据库设计-一对一.png](数据库设计-一对一.png)
## 5）案例
需求：设计专辑评论页面的数据库。
数据库关系如图所示：
![数据库设计-案例示意图.png](数据库设计-案例示意图.png)
